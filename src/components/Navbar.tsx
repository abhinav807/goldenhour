...
export default function Navbar() {
  ...
  return <>
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-200 ${scrolled ? "bg-black/95 backdrop-blur-sm border-b-2 border-sunset" : "bg-black/30"}`} aria-label="Primary navigation">
      <div className="max-w-[1400px] mx-auto px-5 md:px-8 flex items-center justify-between h-16 md:h-20"><a href="#hero" className="font-archivo text-xl md:text-2xl text-offwhite tracking-tighter hover:text-sunset transition-colors" onClick={close}>GOLDEN<span className="text-sunset">.</span>HOUR</a>
