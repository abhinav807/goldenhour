...
export default function FinalCTA() {
  return <section id="register" className="bg-sunset py-24 md:py-36 px-5 md:px-12 relative overflow-hidden border-t-4 border-black">
    ...
    <div className="max-w-[980px] mx-auto relative z-10 reveal text-center">
      ...
      <h2 className="font-archivo text-[16vw] md:text-[9vw] lg:text-[7.5rem] text-black leading-[.79] mb-8">NOT OPEN<br />YET<span className="text-offwhite">.</span></h2>
      <p className="font-dm text-lg md:text-2xl text-black/70 max-w-2xl mx-auto leading-snug">Participant registration will open once the event details are confirmed. Follow GOLDENHOUR for the announcement.</p>
      <div className="flex flex-col sm:flex-row gap-4 mt-10 justify-center"><a href={volunteerUrl} target="_blank" rel="noreferrer" className="brutal-btn px-8 py-5 text-sm md:text-base">VOLUNTEER WITH US <ArrowRight size={18} /></a><a href="mailto:goldenhourdelhi@gmail.com" className="inline-flex items-center justify-center gap-2 font-space text-xs font-bold uppercase underline underline-offset-4 hover:no-underline">Questions? Email us <Mail size={15} /></a></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 border-t-2 border-black/20 pt-7 mt-16 text-center">{[["DATE", "LATE OCT / EARLY NOV"], ["LOCATION", "DELHI / VENUE TBA"], ["DURATION", "10 HOURS"], ["ENTRY", "FREE"]].map(([label, value]) => <div key={label}><p className="font-space text-[10px] text-black/45 uppercase font-bold mb-1">{label}</p><p className="font-space text-xs font-bold text-black">{value}</p></div>)}</div>
    </div>
  </section>;
}
